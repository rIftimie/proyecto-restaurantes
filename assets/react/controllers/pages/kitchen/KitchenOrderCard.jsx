...
function KitchenOrderCard({ order, useStateOrder }) {
  ...
  const fechaFormateada = `${dia.toString().padStart(2, "0")}-${mes
    .toString()
    .padStart(2, "0")}-${year.toString()} ${hora
    .toString()
    ...
    <>
      <section className={classes.join(" ")} style={{ textAlign: "center" }}>
        <div className="card-body">
          ...
          <br />
          <div className="text-center orderImportant">
            Pedido: <br />
            <h5 className="text-start ms-5 orderImportant">{orderProducts}</h5>
          </div>
          <h5 className="text-center orderMesa">Mesa: {order.table}</h5>
          <h5 className="text-center">Fecha: {fechaFormateada}</h5>
          <br />
