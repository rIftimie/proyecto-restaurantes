...
function OrderWaiterContainer({ useStateOrder }) {
  const orders = useStateOrder.orders.filter(order => order.status===0 || order.status===3);
  
  const renderOrders = orders.map((order) => <OrderWaiterCard order={order} useStateOrder={useStateOrder}  />);
